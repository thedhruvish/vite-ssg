...
async function runPrerender() {
  ...
  try {
    ...
    console.error(
      "❌ Failed to read dist/index.html. Make sure 'vite build' runs first.",
      err,
    )
    process.exit(1)
  }
  ...
  for (const page of pages) {
    ...
    if (page.prefetch) {
      try {
        ...
        console.warn(
          `[SSG Warning] Prefetch failed for ${page.url}: ${e?.message || e}`,
        )
      }
    }
    ...
    if (page.url !== '/') {
      const directHtmlFile = path.join(
        distDir,
    ...
    console.log(
      `  ✓ Prerendered ${page.url} → ${path.relative(webDir, targetFile)}`,
    )
    count++
  }
